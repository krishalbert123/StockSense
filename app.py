...
def seed_initial_stocks():
    # Try expanded list first, then fallback to original
    paths = [
        os.path.join(os.path.dirname(__file__), "expanded_stocks.json"),
    ...
    for path in paths:
        if os.path.exists(path):
            try:
                ...
                print(f"✓ Seeded {len(stocks_data)} stocks from {path}")
                return
            except Exception as e:
                print(f"Warning: failed to seed from {path}: {e}")
    
    print("Warning: No stock files found for seeding")
...
def popular_stocks():
    """
    Returns a curated list of popular stocks (top US and Indian stocks).
    """
    popular = [
        ("AAPL", "Apple Inc."),
    ...
    return jsonify([{"symbol": s[0], "name": s[1]} for s in popular])
