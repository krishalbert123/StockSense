...
/* ── SEARCHABLE SELECT ─────────────────────────────────────
   ...
   For view/compare/predict: searches ALL available stocks in real-time
   For add/delete: uses watchlist
...
async function loadPopularStocks(key) {
    const drop = document.getElementById(`ss-drop-${key}`);
    if (!drop) return;
    try {
        const res = await fetch("/api/popular-stocks");
        const popular = await res.json();
        drop.innerHTML = popular.map(s => {
        ...
        console.error("Error loading popular stocks:", e);
    }
}
...
async function renderDropdownFromAPI(key, query) {
    ...
    try {
        ...
        drop.innerHTML = `<div class="ss-empty">Search error. Try typing again.</div>`;
    }
}
...
function populateSelects() {
    ...
    // view page - search ALL stocks
    if (!document.getElementById("ss-input-view")) {
        buildSearchSelect("view-select-wrap", "view", "— Type to search stock —", false);
    }
    // compare page - search ALL stocks
    if (!document.getElementById("ss-input-cmpS1")) {
        buildSearchSelect("cmp-s1-wrap", "cmpS1", "— First stock —", false);
    }
    if (!document.getElementById("ss-input-cmpS2")) {
        buildSearchSelect("cmp-s2-wrap", "cmpS2", "— Second stock —", false);
    }
    // predict page - search ALL stocks
    if (!document.getElementById("ss-input-pred")) {
        buildSearchSelect("pred-select-wrap", "pred", "— Type to search stock —", false);
    }
    // manage page - only watchlist
    if (!document.getElementById("ss-input-del")) {
        buildSearchSelect("del-select-wrap", "del", "— Select a stock to delete —", true);
    }
}
